...
metadata = {
    'protocolName': 'CPL protocol 4: Filter first enrichment',
    'author': 'Jacob Sturgess',
    'description': 'Transfer 200ul from each well of first enrichment to a filter plate.',
    'apiLevel': '2.9'
}
...
def run(ctx):
    ...
    wells_to_filter = 36
    full_wells_in_filterplate = 0 # Number of wells in the filter plate which already contain any liquid. OPTIONAL.
    # ------------------------------------------------------------------------------------------------------------------
    ...
    deepwells = prepare_plate(deepwellplate,96)[:wells_to_filter]
    filterwells = prepare_plate(filterplate, 96)[full_wells_in_filterplate : wells_to_filter]

    for deepwell, filterwell in zip(deepwells, filterwells):
        p300.transfer(200, deepwell, filterwell)

    ctx.comment('Protocol complete. Filter plate is ready to be sealed and sterilised by centifugation, then can be cold stored or used in "CPL5 Second Phage Enrichment" protocol.')